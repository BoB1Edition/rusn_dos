...
pub(crate) fn init_ivt(machine: &mut DosMachine) {
    ...
    set_vector(machine, 0x13, 0xF000, 0x0040); // INT 13h: Disk Services
    set_vector(machine, 0x15, 0xF000, 0x0050); // INT 15h: BIOS Extensions
    set_vector(machine, 0x16, 0xF000, 0x0060); // INT 16h: Keyboard Services
    set_vector(machine, 0x1C, 0xF000, 0x0070); // INT 1Ch: User Timer Tick
    set_vector(machine, 0x1F, 0xF000, 0x0080); // INT 1Fh: Video Graphics Table
    set_vector(machine, 0x20, 0xF000, 0x0090); // INT 20h: Program Terminate
    set_vector(machine, 0x21, 0xF000, 0x00A0); // INT 21h: DOS API
    set_vector(machine, 0x2F, 0xF000, 0x00B0); // INT 2Fh: Multiplex
    set_vector(machine, 0x67, 0xF000, 0x00C0); // INT 67h: EMS (LIM 4.0)
