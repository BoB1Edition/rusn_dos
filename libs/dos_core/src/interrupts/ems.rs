...
pub fn handle_int67(machine: &mut DosMachine) {
    match machine.registers.ah() {
        ...
        0x42 => { // Map/Unmap Page (AL=log page, BX=handle, DX=phys page)
        ...
        0x43 => { // Get Unallocated Pages
            machine.registers.set_bx(machine.ems_free_pages);
            machine.registers.set_ah(0x00);
        }
        0x44 => { // Allocate Pages (BX = requested)
        ...
        0x45 => { // Deallocate Pages (DX = handle)
        ...
        0x49 => { // Get Handle Pages (DX = handle)
            let handle = machine.registers.dx();
        ...
        _ => {
            ...
            log::warn!("Unsupported EMS INT 67h / AH={:02X}", machine.registers.ah());
        }
    }
}